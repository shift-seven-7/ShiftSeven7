...
export default function ProfileHeader({ staff, facilityName }) {
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0 text-center sm:text-right">
          ...
          <div className="flex items-center gap-1.5 flex-wrap mt-1.5 justify-center sm:justify-start">
            <Badge variant="outline" className="text-xs">{ROLE_LABELS[staff.role] || staff.role}</Badge>
            {staff.qualification && staff.qualification !== "none" && QUAL_LABELS[staff.qualification] && (
              <Badge variant="secondary" className="text-xs">{QUAL_LABELS[staff.qualification]}</Badge>
            )}
