import { ShieldCheck, RefreshCw, HeartPulse, AlertTriangle, CheckCircle2, Clock } from "lucide-react";
...
function statusFor(dateStr) {
  ...
  if (days < 0) return { level: "expired", label: "פג תוקף", badge: "bg-red-100 text-red-700", border: "border-red-200", icon: AlertTriangle };
  if (days <= 30) return { level: "urgent", label: `בעוד ${days} ימים`, badge: "bg-red-100 text-red-700", border: "border-red-200", icon: AlertTriangle };
  if (days <= 60) return { level: "soon", label: `בעוד ${days} ימים`, badge: "bg-amber-100 text-amber-700", border: "border-amber-200", icon: Clock };
  return { level: "ok", label: `בעוד ${days} ימים`, badge: "bg-green-100 text-green-700", border: "border-green-200", icon: CheckCircle2 };
}

export default function CredentialsReminders({ staff }) {
  const items = CREDENTIALS
    .map(c => ({ ...c, date: staff?.[c.key] }))
    .filter(c => c.date);
