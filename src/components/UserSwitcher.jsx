...
import { ChevronUp, Search, RotateCcw, ShieldCheck, CheckCircle2 } from "lucide-react";
...
export default function UserSwitcher({ collapsed = false }) {
  const { staffList, isAdmin, isImpersonating, effectiveStaff, setImpersonation, clearImpersonation } = useImpersonation();
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="top" align="start" className="w-72 max-h-[70vh] overflow-y-auto">
        {/* Pinned admin return — always visible */}
        <DropdownMenuItem
          onClick={backToAdmin}
        ...
        {groups.length === 0 && (
          <p className="text-xs text-muted-foreground px-3 py-2">לא נמצאו עובדים</p>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">{staffTag(s)} · #{s.employee_id}</p>
                  </div>
