...
const QUAL_LABELS = { shift_supervisor: 'אחמ"ש', lead_dispatcher: "אחר׳ מוקד" };

function toDateStr(d) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

function DayCell({ staffId, date, dayAssignments, templates, onRefresh, isToday }) {
          ...
          >
            <div className="space-y-1 min-h-[44px]">
              {cellAssignments.map(a => {
                ...
                const style = CATEGORY_CELL[tpl?.category] || "bg-gray-100 border-gray-200 text-gray-800";
                ...
                return (
                  <div
                    ...
                    className={cn("px-1.5 py-1 rounded border text-[11px] font-medium cursor-pointer hover:opacity-90 transition-opacity active:scale-95", style)}
                  >
                    <div className="flex items-center gap-1">
                      <span className="font-black text-sm leading-none shrink-0">{a.shift_code}</span>
                      <span className="font-semibold truncate" style={{ fontSize: 10 }}>{tpl?.name}</span>
                      {tpl?.post_number && <span className="font-bold text-[10px] bg-black/10 rounded px-1">ע׳{tpl.post_number}</span>}
                    </div>
                    <div className="opacity-70 mt-0.5" style={{ fontSize: 10 }}>{sTime} – {eTime}</div>
                  </div>
                );
              })}
  ...
  return (
    <tr>
      <td colSpan={colSpan} className="p-0 h-0">
        <div className="flex items-center px-4 py-1.5 bg-muted/40" style={{ borderTop: "2px solid hsl(var(--border))", borderBottom: "2px solid hsl(var(--border))" }}>
...
function StaffRow({ member, weekDays, weekAssignments, templates, onRefresh, todayStr }) {
  return (
    <tr className="hover:bg-muted/10 transition-colors">
      <td
        className="border border-border/30 bg-card px-3 py-2 text-right sticky right-0 z-10"
...
export default function WeeklyMatrix({
  weekDays, hebDays, staff, effectiveFacilityId, hasControlRoom,
  weekAssignments, templates, onRefresh, isGlobalView
}) {
        ...
        <tbody>
          {supervisors.length > 0 && (
            <>
              ...
              {supervisors.map(m => <StaffRow key={m.id} member={m} {...sharedRowProps} />)}
            </>
          )}
          {guards.length > 0 && (
            <>
              <GroupHeader label="מאבטחים" colSpan={colSpan} />
              {guards.map(m => <StaffRow key={m.id} member={m} {...sharedRowProps} />)}
            </>
          )}
          ...
          {hasControlRoom && leadDispatchers.length > 0 && (
            <>
              ...
              {leadDispatchers.map(m => <StaffRow key={m.id} member={m} {...sharedRowProps} />)}
            </>
          )}
          {hasControlRoom && dispatchers.length > 0 && (
            <>
              <GroupHeader label="מוקדניות" colSpan={colSpan} />
              {dispatchers.map(m => <StaffRow key={m.id} member={m} {...sharedRowProps} />)}
            </>
          )}
