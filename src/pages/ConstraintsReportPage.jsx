...
import {
  FileWarning, CheckCircle2, AlertTriangle, Clock,
...
export default function ConstraintsReportPage() {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let reqs = await base44.entities.ShiftRequest.filter({ week_start: weekStart, status: "submitted" });
      if (facilityFilter !== "all") reqs = reqs.filter(r => r.facility_id === facilityFilter);
      return reqs;
    },
  });
  ...
  const { data: assignments = [] } = useQuery({
    ...
    queryFn: async () => {
      const results = await Promise.all(weekDateStrs.map(date =>
        facilityFilter === "all"
          ? base44.entities.ShiftAssignment.filter({ date })
          : base44.entities.ShiftAssignment.filter({ date, facility_id: facilityFilter })
      ));
      return results.flat();
    },
  });
  ...
  const tplName = (id) => templates.find(t => t.id === id);
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(({ request: r, status, assignment }, idx) => {
                const reqTpl = tplName(r.shift_template_id);
                const meta = STATUS_META[status];
                return (
                  <tr key={idx} className={cn("border-b border-border/50",
                    status === "conflict" ? "bg-red-50/40" : status === "fulfilled" ? "bg-green-50/30" : "hover:bg-muted/20")}>
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-2">
                        ...
                        {reqTpl && <span className="text-xs text-muted-foreground">{formatTime(reqTpl.start_time)}–{formatTime(reqTpl.end_time)}</span>}
                      </div>
                    </td>
                    <td className="py-2.5 px-3">
                      {assignment ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-muted-foreground">{formatTime(assignment.actual_start)}–{formatTime(assignment.actual_end)}</span>
                        </div>
                      ) : (
