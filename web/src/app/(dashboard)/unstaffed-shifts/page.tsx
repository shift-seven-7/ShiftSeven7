...
import { AlertCircle, CalendarX, CheckCircle2, ChevronLeft, ChevronRight, Shield } from "lucide-react";
...
function requirementLabel(req: RequirementCounts | null) {
  if (!req) return "";
  const labels: Record<string, string> = { supervisor: 'אחמ"ש', guard: "מאבטח", dispatcher: "מוקדנית" };
  return (Object.keys(req) as (keyof RequirementCounts)[])
    .filter((k) => (req[k] || 0) > 0)
...
export default function UnstaffedShiftsPage() {
  ...
  const effectiveFacility = useMemo(
    () => facilities.find((f) => f.id === effectiveFacilityId),
    [facilities, effectiveFacilityId],
  );
  ...
  // buildRequirementsMap keys by facility CODE (a client-side default table) -
  // DB rows key by facility_id (see docs/MIGRATION_PLAN.md B.2), so resolve
  // each row's facility code before feeding it in.
  const requirementsMap = useMemo(() => {
    const idToCode: Record<string, string> = {};
    facilities.forEach((f) => {
      idToCode[f.id] = f.code;
    });
    const withCode = requirementRecords.map((r) => ({
      ...r,
      day_group: r.day_group as DayGroup,
  ...
  const templateCategoryById = useQuery({
    queryKey: ["shift-templates-categories"],
    queryFn: async () => {
      const supabase = createClient();
      const { data, error } = await supabase.from("shift_templates").select("id, category");
      if (error) throw error;
      const m: Record<string, string> = {};
      data.forEach((t) => (m[t.id] = t.category));
      return m;
    },
  }).data;
  ...
  const poolMap = useMemo(() => {
    ...
    assignments
      ...
      .forEach((a) => {
        const cat = templateCategoryById?.[a.shift_template_id];
        if (!cat) return;
    ...
    return m;
  }, [assignments, templateCategoryById, staffMap]);

  const rows = useMemo(() => {
    const cats = CATEGORY_ORDER.filter((cat) => weekDays.some((d) => getRequirement(effectiveFacility, d, cat, requirementsMap)));
    return cats.map((cat) => {
  ...
  const totalRequiredSlots = rows.reduce((sum, r) => sum + r.days.filter((d) => d.hasReq).reduce((s, d) => s + d.shortage!.totalRequired, 0), 0);
  const totalAssignedSlots = rows.reduce((sum, r) => sum + r.days.filter((d) => d.hasReq).reduce((s, d) => s + d.shortage!.totalAssigned, 0), 0);
  const totalShortage = rows.reduce((sum, r) => sum + r.days.filter((d) => d.hasReq).reduce((s, d) => s + d.shortage!.totalShortage, 0), 0);
  const daysWithGaps = weekDateStrs.filter((date) => rows.some((r) => r.days.find((d) => d.date === date && d.hasReq && d.shortage!.totalShortage > 0))).length;
        ...
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <div className="flex gap-1 bg-muted rounded-lg p-0.5 border border-border">
            <button
              ...
              className={cn("px-3 py-1.5 rounded-md text-xs font-semibold transition-all", showGapsOnly ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground")}
            >
          ...
          {totalShortage > 0 && (
            <Link href="/smart-schedule" className="flex items-center gap-1.5 h-8 px-3 text-xs font-semibold bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors">
                  ...
                  <td className="py-2.5 px-3 sticky right-0 bg-inherit">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      <span className="text-[10px] text-muted-foreground">
                        {requirementLabel(getRequirement(effectiveFacility, weekDays[0], cat, requirementsMap))}
                      </span>
                      {gapCount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-100 text-red-700 font-semibold">{gapCount} פערים</span>}
                    </div>
                  </td>
                    ...
                    return (
                      <td key={date} className={cn("text-center py-2 px-1 border border-border/30", isToday && "ring-1 ring-primary/30")}>
                        {isGap ? (
                          ...
                          <span title={title} className="inline-flex flex-col items-center justify-center w-12 h-10 rounded-lg bg-green-50 text-green-700">
