...
export default function ShiftRequestPage() {
  ...
  const requestMap = useMemo(() => {
    const m: Record<string, (typeof existingRequests)[number]> = {};
  ...
  const availableTemplates: ShiftTemplate[] = useMemo(() => {
    if (!myStaff) return templates;
    return templates.filter((t) => (t.applicable_roles || []).includes(myStaff.role));
  }, [templates, myStaff]);

  const handleSelectTemplate = async (date: string, template: ShiftTemplate) => {
    if (!myStaff) return;
    const supabase = createClient();
    ...
    if (existing) {
      if (existing.shift_template_id === template.id) {
        ...
        await supabase
          .from("shift_requests")
          .update({ shift_template_id: template.id, shift_code: template.code })
          .eq("id", existing.id);
      }
    } else {
  ...
  if (!myStaff) {
    return (
      <div className="min-h-screen flex items-center justify-center text-sm text-muted-foreground" dir="rtl">
          ...
          {weekDays.map((d, i) => {
            ...
            return (
              <div key={dateStr} className="flex flex-col gap-1">
                ...
                >
                  {req && tpl ? (
                    <div className="p-2 flex flex-col items-center justify-center h-full gap-0.5">
                      <span className="font-black text-lg leading-none" style={{ color: colors?.text }}>
                        {tpl.code}
                      </span>
                ...
                {isOpen && (
                  <div className="absolute z-30 mt-1 w-44 bg-card border border-border rounded-xl shadow-xl p-1.5 space-y-1" style={{ position: "relative" }}>
                    {availableTemplates.length === 0 && <p className="text-xs text-muted-foreground p-2">אין תבניות זמינות</p>}
