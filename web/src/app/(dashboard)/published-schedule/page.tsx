...
function exportCSV(
  ...
  templates: Database["public"]["Tables"]["shift_templates"]["Row"][],
  facilityLabel: string,
  staffData: Database["public"]["Tables"]["staff"]["Row"][],
) {
  const BOM = "﻿";
  ...
  const rows = assignments.map((a) => {
    ...
    const liveStaff = staffData.find((x) => x.id === a.staff_id);
    const name = liveStaff?.full_name || a.staff_name;
    return [name, a.date, a.shift_code, formatTime(a.actual_start), formatTime(a.actual_end), facilityLabel, tpl?.post_number || ""]
      .map((v) => `"${v}"`)
      .join(",");
  });
  ...
  const { data: assignments = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (myShifts && myStaffRecord) {
        query = query.eq("staff_id", myStaffRecord.id);
      } else if (!isGlobalView && effectiveFacilityId) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      await exportSchedulePDF({
        ...
        staffData,
        weekLabel,
        facilityName: isGlobalView ? undefined : facilities.find((f) => f.id === effectiveFacilityId)?.name,
      });
    } finally {
      setExporting(false);
    }
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex gap-1 bg-muted rounded-lg p-0.5 border border-border">
              <button
                ...
                className={cn("px-3 py-1 rounded-md text-xs font-semibold transition-all", myShifts ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground")}
              >
                המשמרות שלי
              </button>
              ...
              <button
                ...
                className={cn("px-3 py-1 rounded-md text-xs font-semibold transition-all", isGlobalView ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground")}
              >
                כלל הצוות
              </button>
            </div>
            <Button size="sm" variant="outline" className="gap-1.5 text-xs h-8" onClick={handleExportPDF} disabled={exporting || active.length === 0}>
            ...
            <Button
              ...
              onClick={() => exportCSV(active, templates, isGlobalView ? "" : facilities.find((f) => f.id === effectiveFacilityId)?.name || "", staffData)}
            >
              ...
              {staffList.map(([staffId, staffName], ri) => {
                ...
                return (
                  <Fragment key={staffId}>
                    ...
                    <tr>
                      <td className="sticky right-0 z-10 text-xs font-semibold px-3 py-2 border border-gray-400 text-right whitespace-nowrap" style={{ background: ri % 2 === 0 ? "#f3f4f6" : "#e9eaec", width: 140 }}>
                        {staffName}
                      </td>
                      {sortedDates.map((date) => {
                        ...
                        return (
                          <td key={date} className="border border-gray-400 text-center p-1 text-xs" style={{ background: color || (ri % 2 === 0 ? "#ffffff" : "#f9fafb") }}>
                            {a && tpl && (
