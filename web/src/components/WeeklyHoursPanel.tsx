...
import { AlertTriangle, CheckCircle2, ChevronDown, ChevronUp, Clock, Timer, TrendingUp, Users } from "lucide-react";
...
function StatCard({ icon: Icon, label, value, sublabel, color }: { icon: typeof Users; label: string; value: React.ReactNode; sublabel?: string; color: string }) {
  return (
...
export default function WeeklyHoursPanel() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/reports/weekly-hours?${params}`);
      if (!res.ok) throw new Error("Failed to load weekly hours");
      return (await res.json()) as WeeklyHoursResponse;
    },
  });
    ...
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-5">
        ...
        <div className="flex gap-1 bg-muted rounded-lg p-0.5 border border-border">
          <button
            ...
            className={cn("px-3 py-1 rounded-md text-xs font-semibold transition-all", facilityFilter === "all" ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground")}
          >
            כל המתקנים
          </button>
          {facilities.filter((f) => f.status !== "inactive").map((f) => (
            <button
              ...
              className={cn("px-3 py-1 rounded-md text-xs font-semibold transition-all", facilityFilter === f.id ? "bg-primary text-primary-foreground shadow" : "text-muted-foreground hover:text-foreground")}
            >
              {f.name}
            </button>
          ))}
      ...
      {!isLoading && staffHours.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          ...
          <StatCard
            icon={violationCount > 0 ? AlertTriangle : CheckCircle2}
            label="חריגות מכסה"
              ...
              {staffHours.map((h) => {
                ...
                return (
                  <>
                    <tr
                      key={h.staff_id}
                      className={cn("border-b border-border/50 transition-colors cursor-pointer", h.is_over_limit ? "bg-red-50/40" : "hover:bg-muted/30")}
                      onClick={() => setExpandedId(isExpanded ? null : h.staff_id)}
                    >
                      <td className="py-2.5 px-2 text-muted-foreground">{isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</td>
                      <td className="py-2.5 px-3 font-medium">{h.staff_name}</td>
                      <td className="py-2.5 px-3 text-muted-foreground hidden sm:table-cell">{h.role === "guard" ? "מאבטח" : h.role === "dispatcher" ? "מוקדן" : h.role}</td>
                      ...
                      <td className="py-2.5 px-3 font-bold tabular-nums">
                        {h.total_hours}
                        <span className="text-[10px] font-normal text-muted-foreground">/{maxHours}</span>
                      </td>
                      <td className="py-2.5 px-3">
                        {h.is_over_limit ? (
                        ...
                        <td colSpan={6} className="p-4">
                          ...
                          <div className="grid grid-cols-7 gap-2">
                            {weekDays.map((d, i) => {
                              ...
                              return (
                                <div key={i} className={cn("rounded-lg border p-2 text-center transition-all", hours > 0 ? "border-primary/30 bg-card" : "border-border/50 bg-muted/30", isToday && "ring-2 ring-primary/40")}>
                                  <p className="text-[10px] font-semibold text-muted-foreground">{HEB_DAYS_SHORT[i]}</p>
                                  <p className="text-[10px] text-muted-foreground mb-1">
                                    {d.getDate()}/{d.getMonth() + 1}
                                  </p>
                                  <p className={cn("text-sm font-bold tabular-nums", hours > 0 ? "text-foreground" : "text-muted-foreground/40")}>{hours}</p>
