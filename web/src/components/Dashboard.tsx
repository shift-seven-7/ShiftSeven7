...
import { buildPool, buildRequirementsMap, CAPABILITY_LABELS, computeShortage, getRequirement } from "@/lib/staffingRequirements";
import { createClient } from "@/lib/supabase/client";
import type { Database } from "@/lib/supabase/database.types";
import { useQuery } from "@tanstack/react-query";
import { AlertTriangle, ArrowLeft, Building2, CalendarX2, CheckCircle2, ClipboardList, Clock, MapPin, Shield, Users } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "requests" | "hours">("overview");
  ...
  const requirementsMap = useMemo(() => {
    ...
    facilities.forEach((f) => (idToCode[f.id] = f.code));
    const withCode = requirementRecords.map((r) => ({
      ...r,
      day_group: r.day_group as "weekday" | "friday" | "saturday",
      category: r.category as "morning" | "afternoon" | "night",
      facility_code: idToCode[r.facility_id],
    }));
    return buildRequirementsMap(withCode);
  }, [requirementRecords, facilities]);
  ...
  const morningCheck = useMemo(() => {
    ...
    const issues: { facility: (typeof facilities)[number]; shortage: ReturnType<typeof computeShortage> }[] = [];
    facilities
      ...
      .forEach((f) => {
        ...
        if (shortage.totalShortage > 0) issues.push({ facility: f, shortage });
      });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <PageHeader title="לוח בקרה" description={`סקירה ליום ${new Date().toLocaleDateString("he-IL", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}`}>
        {emergencyMode && (
      ...
      <div className="flex gap-1 bg-muted/50 rounded-xl p-1 mb-6 border border-border w-fit">
        <button onClick={() => setActiveTab("overview")} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${activeTab === "overview" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
          סקירה כללית
        </button>
        <button onClick={() => setActiveTab("requests")} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-1.5 ${activeTab === "requests" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
          <ClipboardList className="w-3.5 h-3.5" />
          בקשות עובדים
        </button>
        <button onClick={() => setActiveTab("hours")} className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-1.5 ${activeTab === "hours" ? "bg-card shadow text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
      ...
      {activeTab === "overview" && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatCard loading={isLoading} icon={Users} tone="blue" to="/staff" label="מאבטחים פעילים" value={guards.length} sublabel={`${guards.filter((g) => g.qualification === "shift_supervisor").length} אחמ"שים`} />
            <StatCard loading={isLoading} icon={Users} tone="purple" to="/staff" label="מוקדנים" value={dispatchers.length} sublabel={`${dispatchers.filter((d) => d.qualification === "lead_dispatcher").length} אחראיות מוקד`} />
            <StatCard loading={isLoading} icon={Shield} tone="green" to="/smart-schedule" label="משמרות היום" value={scheduledToday.length} sublabel={emergencyOverrides.length > 0 ? `${emergencyOverrides.length} חריגות חירום` : "אין חריגות"} />
            <StatCard loading={isLoading} icon={MapPin} tone="amber" to="/posts" label="עמדות פעילות" value={posts.filter((p) => p.status === "active").length} sublabel={`ב-${facilities.length} מתקנים`} />
          </div>
                  ...
                  {morningCheck.issues.map(({ facility, shortage }) => (
                    <div key={facility.id} className="p-3 rounded-lg bg-amber-50 border border-amber-200">
                      <div className="flex items-start gap-2">
                        ...
                        <Link href="/smart-schedule" className="shrink-0 flex items-center gap-1 text-[11px] font-semibold text-amber-800 hover:text-amber-900 hover:underline">
                          לסידור
                    ...
                    return (
                      <div key={f.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 border border-transparent">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-9 h-9 rounded-lg bg-card border border-border flex items-center justify-center shrink-0 text-xs font-bold text-muted-foreground">{f.code}</div>
                          <p className="text-sm font-semibold truncate">{f.name}</p>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-muted-foreground shrink-0">
                          <span className="flex items-center gap-1">
                            <Users className="w-3.5 h-3.5" />
                            {facilityStaff.length}
                          </span>
                          <span className="flex items-center gap-1">
                            <MapPin className="w-3.5 h-3.5" />
                            {facilityPosts.length}
                          </span>
                          <span className="flex items-center gap-1">
          ...
          <div className="bg-card border border-border rounded-xl p-6 mt-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {!isLoading && scheduledToday.length > 10 && <span className="text-xs text-muted-foreground">מציג 10 מתוך {scheduledToday.length}</span>}
            </div>
            {isLoading ? (
              <div className="space-y-2">
                {[0, 1, 2].map((i) => (
                        ...
                        <tr key={a.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                          <td className="py-2.5 px-3">
                            <div className="flex items-center gap-2.5 min-w-0">
                              <div className="w-7 h-7 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold text-accent-foreground shrink-0">{initials(a.staff_name)}</div>
