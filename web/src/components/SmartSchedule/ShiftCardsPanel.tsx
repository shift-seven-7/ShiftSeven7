...
const CATEGORY_LABELS: Record<string, string> = { morning: "בוקר", afternoon: "צהריים", night: "לילה" };
const CATEGORY_DOT: Record<string, string> = { morning: "bg-amber-400", afternoon: "bg-blue-400", night: "bg-violet-400" };
const CATEGORY_ORDER = ["morning", "afternoon", "night"];

export default function ShiftCardsPanel({
  templates,
  hasControlRoom,
}: {
  ...
  const grouped = CATEGORY_ORDER.reduce<Record<string, ShiftTemplate[]>>((acc, cat) => {
    ...
    <div className="w-48 shrink-0 flex flex-col overflow-hidden">
      <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm flex flex-col h-full">
        ...
        <Droppable droppableId="panel" isDropDisabled>
          {(provided) => (
            <div ref={provided.innerRef} {...provided.droppableProps} className="flex-1 overflow-y-auto p-2 space-y-4">
              ...
              {flat.length === 0 && <p className="text-xs text-muted-foreground text-center py-4">אין תבניות</p>}
            </div>
          )}
        </Droppable>
      </div>
