// Ported from the old app's lib/exportSchedule.js — same off-screen-HTML +
// html2canvas + jsPDF approach (a real PDF table is nicer, but this mirrors
// the original's colored shift-template cells exactly, which a text-table
// export wouldn't reproduce without re-deriving that logic). Client-only:
// touches document/window, so only ever called from a 'use client' page.

import { jsPDF } from 'jspdf';
import html2canvas from 'html2canvas';
import type { FacilityRow, ShiftAssignmentRow, ShiftTemplateRow, StaffRow } from '@/types/database.types';
...
function rankOf(staffId: string, staffData: StaffRow[]): number {
...
export async function exportSchedulePDF({
  ...
  weekLabel,
  facilityName,
}: ExportSchedulePDFOptions): Promise<void> {
  ...
  const entries = Object.entries(map).sort(
    (a, b) => rankOf(a[0], staffData) - rankOf(b[0], staffData) || a[1].localeCompare(b[1], 'he')
  );

  const dates = [...new Set(active.map((a) => a.date))].sort();
  const cellMap: Record<string, ShiftAssignmentRow> = {};
  active.forEach((a) => {
    cellMap[`${a.staff_id}::${a.date}`] = a;
  });

  const container = document.createElement('div');
  container.style.cssText =
    'position:fixed; left:-9999px; top:0; direction:rtl; background:#fff; padding:24px; font-family:Arial,Helvetica,sans-serif;';
...
/** Export the visible assignments as a CSV file (Excel-friendly, UTF-8 BOM). */
